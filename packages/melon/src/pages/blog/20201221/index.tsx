...
export const Article: FC = () => (
  <article id="markdown-root">
    ...
    <p>
      ...
      やっていて楽しい + 学習する余地がありそうな no dependencies で blog を作ることにしました．
    </p>
...
`}</code></pre>
    ...
    <p>
      ...
      jsx 自体を作るのは思っている以上に簡単だったが，どちらかというと html および その周辺の知識が乏しさに気がつくきっかけとなった<br />
...
<a href="https://html.spec.whatwg.org/multipage/dom.html#global-attributes">Global Attributes</a> や，<a href="https://html.spec.whatwg.org/multipage/microdata.html#microdata">マイクロデータ</a>， <a href="https://www.w3.org/TR/wai-aria-1.1/">WAI-ARIA</a>，各要素のセマンティクス，正しい html とはなにか，いかに普段我々の書く html が雑なものかを考えさせられるきっかけになった．</p>
    ...
    <p>
      ...
      コンポーネント軸で開発をすすめると セレクタ詳細度 がごちゃついて辛いのはわかるけれどそれを解決するのが CSS-in-JS なのかは割と今でも疑問．<br />
      ↓ こんな感じで一応普通に使える
      </p>
...
\``}</code></pre>
    ...
    <h3>ログ基盤の構築</h3>
    <p>
      インフラ周りのツールを自作するかどうかは悩んでいるが，もうリリースしてしまったので取り急ぎ
    ...
    <p>
      ...
      この開発を通して，世の中に出ている ライブラリがどのように作られていたりとか，html の仕様とか学習できたのじゃないかと思います．
      そして，いかに標準APIを使いこなせていないか，知らないかが明らかになりました．
      また，ライブラリを採択するときにも，内部実装をみて筋が良いかどうかを判断するというのがこの開発を通して増えてきたと思う．
      例えば，正規表現ベースのマークダウン系のライブラリだったり，オブザーバブルパターンを実装するためだけに Proxy を使っているものだったりと，「本当にそれ必要？」ってより具体的に問うことが以前に比べると成長できた点じゃないかなとおもいます．
    </p>
    <p>
      この開発では，whatwg の仕様を読むことのきっかけにはなったが，肝心の JavaScript の仕様を読むきっかけにはならなかったので，
      今後は，JavaScript の仕様にも目を通して手続き一つ一つにも疑問を問いかけながら開発を楽しんでいきたいなと思いました．
    </p>
    <p>
